...
struct hrtimer palmtimer;	
ktime_t palm_time;
int palm_touch_pressed = false;
#endif
...
#if defined(CONFIG_S5PC110_FLEMING_BOARD) || defined(CONFIG_S5PC110_SIDEKICK_BOARD)
...
void TSP_forced_release(void)
{
	...
	for ( i= 1; i<MAX_USING_FINGER_NUM; ++i )
	{
		...
		input_report_abs(qt602240->input_dev, ABS_MT_TOUCH_MAJOR, fingerInfo[i].pressure);	// 0捞搁 Release, 酒聪搁 Press 惑怕(Down or Move)
...
//100614 sooo.shin workaround for touch lockup(I9000)
...
#if defined(CONFIG_S5PC110_SIDEKICK_BOARD)
static enum hrtimer_restart palm_timer_func(struct hrtimer *timer)
{
	printk("[%s]...\n", __func__);
	
	if(palm_touch_pressed == true)
	{
		printk("[%s] Force palm touch to be released... \n", __func__);
		TSP_forced_release_forOKkey();
		//calibrate_chip();

		palm_touch_pressed = false;
	}
...
//						calibrate_chip();
					...
					#if defined(CONFIG_S5PC110_SIDEKICK_BOARD)	
						palm_time = ktime_set(0,0);
						palm_time = ktime_add_us(palm_time,3000000);
						hrtimer_start(&palmtimer,palm_time,HRTIMER_MODE_REL);

						palm_touch_pressed = true;
					#endif
				}
			}	
			...
			if(quantum_msg[0] < 2  || quantum_msg[0] >= 12) {
				...
				else if((quantum_msg[0] == 1) && ((quantum_msg[1]&0x10)==0 && (quantum_msg[1] != 0x40))/* && (cal_check_flag==1)*/){
...
//					cal_check_flag = 2u;
				...
				if (cal_check_flag || (quantum_msg[0] == 0x8F)) {
						check_chip_calibration(1);
				}				
...
#ifdef QT_FIRMUP_ENABLE
{
	...
	#if defined(CONFIG_S5PC110_SIDEKICK_BOARD)
		hrtimer_init(&palmtimer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
		palmtimer.function = palm_timer_func;
	#endif
...
{	// v1.2 = 18 , v1.4 = 20 , v1.5 = 21
    ...
    return sprintf(buf, "0X%x", tsp_version );
}
...
static ssize_t firmware_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if ( value == 1 )	// auto update.
	{
		...
		if(((tsp_version != 0x14)&&(tsp_version <0x16))||((tsp_version==0x16)&&(g_build==0xaa)))
		{			
...
{
        printk(TCH_KERN_DEBUG "[TSP] %s is called.. is_inputmethod=%d\n", __func__, is_inputmethod);
        if( is_inputmethod )
        {
                *buf = '1';
        }
...
ssize_t set_tsp_for_inputmethod_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	disable_irq(qt602240->client->irq);
		...
		printk(TCH_KERN_DEBUG "[TSP] Set TSP inputmethod IN \n");		 
		...
		printk(TCH_KERN_DEBUG "[TSP] Set TSP inputmethod OUT \n");	
